...
export function AgentInvocationPanel() {
  ...
  useEffect(() => { void listInvocationTargets().then((next) => { setTargets(next); setSelected(next[0]?.agentId ?? ''); }).catch(() => setError('Test backend status is unavailable.')).finally(() => setLoading(false)); }, []);
  ...
  return <div className="agent-invocation" aria-labelledby="agent-invocation-heading">
    ...
    {loading ? <p role="status">Checking test backend…</p> : targets.length === 0 ? <p>No executable test backend is configured.</p> : <form onSubmit={submit}>
