...
describe('Knowledge validation and explicit promotion', () => {
  ...
  it('requires explicit approval of a current preview and creates one note on repeat save', () => {
    ...
    expect(() => f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: false, previewToken: preview.token })).toThrow(/must review/);
    expect(() => f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: true, previewToken: 'stale' })).toThrow(/changed/);
    ...
    const repeated = f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: true, previewToken: f.knowledge.preview('workspace-a', record.id).token });
  ...
  it('retains a failed candidate and retries with its stable identity and approval timestamp', () => {
    ...
    const failed = f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: true, previewToken: f.knowledge.preview('workspace-a', record.id).token });
    expect(failed.status).toBe('failed'); expect(failed.savedAt).toBeNull(); expect(failed.content).toBe(record.content);
    fault.mockRestore();
    const saved = f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: true, previewToken: f.knowledge.preview('workspace-a', record.id).token });
  ...
  it('does not write a note if durable approval cannot be recorded', () => {
    ...
    expect(() => f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: true, previewToken: f.knowledge.preview('workspace-a', record.id).token })).toThrow('Database unavailable');
  ...
  it('recovers the file-created/database-unconfirmed gap on explicit retry without a second note', () => {
    ...
    expect(() => f.knowledge.save(LOCAL_HAN, 'workspace-a', record.id, { approved: true, previewToken: f.knowledge.preview('workspace-a', record.id).token })).toThrow('database write failed');
  ...
  it('keeps candidates available with an unconfigured connector and blocks changed destinations', () => {
    const f = fixture(); const service = new KnowledgeService(f.knowledgeRepository, f.workspaces, f.outcomeRepository, new ObsidianConnector(), f.governance);
